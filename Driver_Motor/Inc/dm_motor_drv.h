...
typedef struct
{
	FDCAN_HandleTypeDef *hcan;//新增can句柄
...
void dm_motor_ctrl_send(Damiao_Motor_t *motor);
void dm_motor_enable(Damiao_Motor_t *motor);
...
void mit_ctrl(hcan_t* hcan, Damiao_Motor_t *motor, uint16_t motor_id, float pos, float vel,float kp, float kd, float tor);
